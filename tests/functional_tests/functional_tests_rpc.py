...
# a main offline juded, does most of the tests
# a restricted RPC juded setup with RPC payment
# two local online judeds connected to each other
N_JUDEDS = 4
...
wallet_base = [builddir + "/bin/jude-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
