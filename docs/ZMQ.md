# The Current/Future Status of ZMQ in Jude

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Jude
...
matching supported by both Jude and ZMQ. The `format`, `context` and `event`
...
The Jude daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Jude daemon also ensures that
...
The Jude daemon will send a `txpool_add` pub exactly once for each
