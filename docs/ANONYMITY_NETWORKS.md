...
Juded does not support synchronizing the blockchain over onion or I2P hidden services.
You may sync the blockchain using a socks proxy.

```bash
juded --proxy 127.0.0.1:9050 --p2p-bind-ip 127.0.0.1 --no-igd
```
...
which tells `juded` to connect to ".onion" P2P addresses using a socks
...
received at address "5tymba6faziy36md5ffy42vatbjzlye4vyr3gyz6lcvdfximnvwpmwqd.onion:18084" and forwarded to `juded`
...
forwarded to `juded` localhost port 18085. Using `tx-proxy`(required), these
...
I2P implementation.  
You can find guides for i2pd [here](https://docs.getjude.org/running-node/juded-tori2p/#__tabbed_1_2).
