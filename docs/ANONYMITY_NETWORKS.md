...
is made or until `judecoind` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `judecoin-wallet-cli` and
`judecoin-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
judecoind does not support synchronizing the blockchain over onion or I2P hidden services.
You may sync the blockchain using a socks proxy.

```bash
judecoind --proxy 127.0.0.1:9050 --p2p-bind-ip 127.0.0.1 --no-igd
```
...
`--tx-proxy` which tells `judecoind` the ip/port of a socks proxy provided by a
...
which tells `judecoind` to connect to ".onion" P2P addresses using a socks
...
`--anonymous-inbound`. This option tells `judecoind` the inbound address, network
...
which tells `judecoind` that a max of 25 inbound Tor connections are being
received at address "5tymba6faziy36md5ffy42vatbjzlye4vyr3gyz6lcvdfximnvwpmwqd.onion:18084" and forwarded to `judecoind`
...
forwarded to `judecoind` localhost port 18085. Using `tx-proxy`(required), these
...
`judecoind` RPC port - which is independent from the configuration for incoming
...
```text
HiddenServiceDir /var/lib/tor/data/jude-rpc
...
### Intermittent Judecoin Syncing

If a user only runs `judecoind` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `judecoind` as often as possible to conceal when transactions are being sent.
...
`judecoind` currently selects two outgoing connections every 5 minutes for
...
`judecoind` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `judecoind` should likely disconnect
