...
gsigs = 'https://github.com/jude-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if os.path.isdir('jude'):
        # Remove the potentially stale jude dir. Otherwise you might face submodule mismatches.
        subprocess.check_call(['rm', 'jude', '-fR'])
    subprocess.check_call(['git', 'clone', args.url, 'jude'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/jude/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'jude='+args.commit, '--url', 'jude='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/jude-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/jude/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/jude/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/jude-project/jude', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/jude', exist_ok=True)
    os.chdir('builder/inputs/jude')
    if args.pull:
